...
public class OnboardingRestController {
    ...
    @Operation(summary = "Issue a preflight onboarding token",
            description = "Call once when the onboarding page loads (GET so it's exempt from CSRF, " +
                    "since this is the first request a fresh client makes). Sets a short-lived " +
                    "httpOnly cookie that authenticates every /onboarding/** call for the rest of " +
                    "the funnel (create, validate, register), and — as a side effect of going " +
                    "through the security filter chain — primes the XSRF-TOKEN cookie the frontend " +
                    "must echo back as X-XSRF-TOKEN on POST calls.")
    @GetMapping("/token")
    public ResponseEntity<?> issueToken(HttpServletRequest request, HttpServletResponse response) {
        ...
        ResponseCookie cookie = ResponseCookie.from(onboardingTokenService.getOnboardingSessionTokenCookieName(), rawToken)
                .httpOnly(true)
        ...
        // Written directly on the servlet response (addHeader appends) rather than via
        // ResponseEntity.header(...), which replaces the whole Set-Cookie header list and
        // would wipe out the XSRF-TOKEN cookie CsrfCookieFilter already added.
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Created", content = @Content(mediaType = "application/json", schema = @Schema(implementation = Onboarding.class))),
            @ApiResponse(responseCode = "400", description = "Bad Request", content = @Content),
            @ApiResponse(responseCode = "403", description = "Forbidden - missing, invalid, or expired onboarding token", content = @Content),
            @ApiResponse(responseCode = "409", description = "Conflict - onboarding already exists for company_email", content = @Content),
            @ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content)
    })
    @io.swagger.v3.oas.annotations.parameters.RequestBody(
            description = "Onboarding request payload",
            required = true,
            content = @Content(mediaType = "application/json", schema = @Schema(implementation = OnboardingRequest.class))
    )
    @PostMapping
    public ResponseEntity<?> onboardUser(@Valid @RequestBody OnboardingRequest request) {
        Onboarding entity = Onboarding.builder()
    ...
    @Operation(summary = "Validate the emailed OTP code", description = "Checks the code against the " +
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "Bad Request - invalid/expired/mismatched code", content = @Content),
            @ApiResponse(responseCode = "403", description = "Forbidden - missing, invalid, or expired onboarding token", content = @Content)
    })
    @PostMapping("/{type}/validate")
    public ResponseEntity<?> validateOtp(@PathVariable String type, @Valid @RequestBody OtpValidationRequest request) {
        onboardingTokenService.validateOtp(
                new OnboardingTokenService.OtpValidationCommand(request.companyEmail(), type, request.code()));
        return ResponseEntity.ok(Map.of("status", "verified"));
    }

    @Operation(summary = "Complete tenant account registration", description = "Creates the tenants_registration " +
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Created", content = @Content(mediaType = "application/json", schema = @Schema(implementation = AccountRegistrationResponse.class))),
            @ApiResponse(responseCode = "400", description = "Bad Request", content = @Content),
            @ApiResponse(responseCode = "403", description = "Forbidden - missing, invalid, or expired onboarding token", content = @Content),
            @ApiResponse(responseCode = "409", description = "Conflict - not yet OTP-verified, or already registered", content = @Content)
    })
