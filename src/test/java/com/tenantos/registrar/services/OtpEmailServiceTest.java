...
class OtpEmailServiceTest {
    ...
    private static final String VERIFY_URL_TEMPLATE = "http://localhost:3000/onboarding/verify?email={email}&code={code}";

    @Mock
    private OnboardingOtpRepository repository;
    @Mock
    ...
    void setUp() {
        ...
        when(repository.save(any(OnboardingOtp.class))).thenAnswer(invocation -> invocation.getArgument(0));
    }
    ...
    @Test
    void generateAndSend_persistsHashedCode_andRendersTheMatchingRawCodeAndVerifyLink() throws Exception {
        ...
        assertThat(verifyUrl)
                .isEqualTo("http://localhost:3000/onboarding/verify?email=a%40example.com&code=" + emailedCode);
    }
